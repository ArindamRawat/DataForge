...
@app.route('/fetch-dataset/<file_id>', methods=['GET'])
def fetch_dataset(file_id):
    try:
        ...
        # Debug: Print the DataFrame to check for issues
        ...
        try:
            ...
            return jsonify({"error": "Error processing the dataset"}), 500
...
def generate_default_row(address, collection_name, columns):
    """
    Generate a default row with 0 for all metrics except for `collection_name` 
    and `contract_address`, which retain their respective values.
    """
    default_row = {"contract_address": address, "collection_name": collection_name}
    
    # Set default values to 0 for all other columns
    default_row.update({column: 0 for column in columns if column not in default_row})
...
if __name__ == '__main__':
    ...
    print(f"Server is available at http://{ip_address}:5000")
